...
## 関連Context（依存関係ではないもの）

- study-log Context: 同じタスク・単元配下では、生徒が単元学習に取り組んだ開始・終了時刻を記録する学習ログ機能が並行して提供されている（Rails現行仕様書「関連機能」参照）。question-answering ContextとStudy-log Contextは同じタスク・単元を対象とするが、互いのデータを参照・更新することはなく、それぞれ独立して「解答の正誤・履歴」「学習時間の記録」という別の業務関心を扱う。そのためBounded Contextとしては分離し、依存関係としては扱わない。
