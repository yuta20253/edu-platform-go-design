...
- 業務ルール: 直近インポート履歴の取得対象を問題インポート（`import_type: question`）に限定する。この絞り込みは入力検証ではなく検索条件として扱う
...
- 対象テーブル: users, user_roles
- 操作種別: 参照（集計）
- 主な検索条件・絞り込み条件: ロール（user_roles.name）ごとにグルーピングして件数をカウントする
...
- 目的: ロール別集計、総問題数集計、直近の問題インポート履歴取得（`import_type`絞り込み・上位5件・降順ソート）の正確性を検証する。特に生徒インポートの履歴が結果に混入しないことを重点的に検証する
