...
- 管理対象: 順位算出に必要な集計データ（対象コース・単元に解答履歴を持つ生徒ごとの正答率）
...
- 補足: 比較指標は、対象コース・単元に対する解答の正答率（正答数÷解答数）であり、理解度スコア（UnderstandingScoreRepositoryが扱う集計データ）や成績平均（GradeRepositoryが扱う集計データ）とは独立した算出結果である
