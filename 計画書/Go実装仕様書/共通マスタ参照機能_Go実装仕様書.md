...
住所検索の`prefecture_id`未指定エラーのみ、他機能の入力検証エラー（多くは422）とは異なり②「17. Error設計」により400を維持する（フロントエンド互換性優先。アーキテクチャ規約「15. 今後の課題」に挙げられているプロジェクト全体のステータスコード不統一問題であり、本機能単独では解決しない）。
