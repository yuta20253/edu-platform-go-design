...
対象外。②に本機能でのMail・Cache・Queue連携要件の記載はない。予約配信の自動切替は行わない（手動のpublishのみ）。②「18. Domain Event」の設計判断のとおり、予約状態のお知らせを配信予定日時に自動的に配信する処理（定期ジョブ・ポーリングワーカー・`jobs`テーブルへの登録・Domain Event）は、Rails現行には存在するが、本書では実装対象に含めない。`scheduled_at`は配信予定の記録としてEntityに保持するのみで、状態を変える契機にはしない。
...
|`PublishAdminAnnouncementUseCase`|draft/scheduledから配信できること／`scheduled_at`を過ぎたscheduled状態のお知らせも、配信操作を行うまでscheduledのままであること（自動で配信状態に切り替わらないこと）／既に配信済みの場合にエラーになること／他の管理者が発信したお知らせの配信が`ErrAnnouncementOperationForbidden`で拒否されること／発信者が無効化されている場合は他の管理者による配信が許可されること|
...
|予約状態のお知らせの手動配信|`status=scheduled`で作成したお知らせが、`scheduled_at`に達しても自動では配信状態にならず、`publish`操作でのみ配信状態になること|
...
|予約配信の自動切替（定期ジョブ・ワーカー・予約日時到達のDomain Event）を実装対象に含めない|②「18. Domain Event」「設計差分管理」の設計判断（管理者お知らせ管理では、予約状態のお知らせの配信を、発信者による手動のpublishのみとする。Rails現行の自動配信ジョブは再現しない）|②の設計判断に従う（推測ではない）|
