...
|操作|概要|
|-|-|
|表示|ユーザー種別ごとの件数と最近の問題インポート履歴を取得する|
...
システム全体のユーザー数と最近の問題インポート状況を管理者用ダッシュボードとして取得する。
...
2. `ImportHistory.question.order(created_at: :desc).limit(5)` で最近のインポート履歴を取得する。このとき、インポート種別が「問題インポート」であるものに絞り込まれる。
3. `stats` と `recent_imports` を JSON で返却する。

### 業務ルール

- 管理者はすべての高校・ユーザーを集計対象とする。
- ユーザー種別ごとの人数には、無効化済み（`deleted_at`あり）のユーザーも含まれる（`active`による除外は行わない）。
- 最近のインポート履歴には問題インポートの履歴のみが表示され、生徒インポートの履歴は含まれない。
...
- 主なカラム: `id`, `user_id`, `unit_id`, `file_name`, `status`, `import_type`, `total_count`, `success_count`, `error_count`, `created_at`
- リレーション: `belongs_to :user`, `belongs_to :unit`
- `import_type` によりインポート種別（問題インポート／生徒インポート）を区別しており、ダッシュボードに表示される「最近のインポート履歴」は問題インポートの履歴のみを対象とする。
