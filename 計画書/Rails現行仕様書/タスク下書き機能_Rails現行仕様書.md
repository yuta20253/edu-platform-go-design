...
- `completed_at`（完了日時）も同様に保持している項目だが、下書きタスクの作成処理では値を設定せず、常に未設定のまま保存される。現行実装には、下書きタスクを完了状態に変更する操作自体（更新・削除を含む）が存在しないため、この機能のスコープ内外を問わず、`completed_at` を更新する操作は提供されていない。
