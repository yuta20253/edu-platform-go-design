...
|更新|下書き・予約状態のお知らせの内容や状態を更新する|
|削除|未配信のお知らせを削除する|
|配信|下書き・予約状態のお知らせを配信（公開）する|
...
5. 下書きまたは予約状態のお知らせは、内容や状態を更新できる。
6. 下書きまたは予約状態のお知らせは配信操作によって公開状態に切り替えられる。公開されると、その時点の日時が配信日時として記録される。
7. 未配信のお知らせは削除できる。配信済みのお知らせは削除・編集できない。
8. 高校ごとのお知らせ一覧画面では、指定した高校を対象に含むお知らせのみが表示される。
...
2. 入力された項目のみを更新する。
3. 状態を変更する場合は、現在の状態から遷移可能な状態であるかを検証する。
...
|条件|HTTP Status|内容|
|-|-|-|
|バリデーション失敗（不正な状態遷移・配信済みの更新など）|422|`errors` を返却|
|対象お知らせが存在しない|404|対象お知らせなし|
...
|条件|HTTP Status|内容|
|-|-|-|
|配信済みのお知らせを削除しようとした場合|422|`errors` を返却|
|対象お知らせが存在しない|404|対象お知らせなし|
...
1. 管理者が発信者であるお知らせの中から対象を取得する。
2. すでに配信済みであればエラーとする。
3. 状態を `published` に更新し、配信予定日時をクリアする。このとき配信日時としてその時点の日時が記録される。
...
- 配信操作（`publish`）が実行されると状態が `published` になり、その時点の日時が配信日時として記録される。

---

# 8. 権限制御

- 管理者は、管理者ロールのユーザーが発信者であるお知らせを参照・作成・更新・削除・配信できる。
- 配信済みのお知らせは編集・削除の対象外となる。
...
- 確認できない事項：予約状態（`scheduled_at`）のお知らせを配信予定日時に自動的に配信状態へ切り替える非同期処理（バッチ・ジョブ等）は、調査した範囲のコードには見当たらない。配信状態への変更は `publish` 操作が明示的に呼び出された場合にのみ行われる。
